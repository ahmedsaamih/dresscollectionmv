...
async function refreshSms(): Promise<{ checked: number; updated: number }> {
  ...
  for (const log of pending) {
    ...
    try {
      ...
      console.error(`[notifications/refresh] sms status check failed for ${log.id}`, e);
    }
  }
...
async function refreshEmail(): Promise<{ checked: number; updated: number }> {
  ...
  for (const log of pending) {
    ...
    try {
      ...
      console.error(`[notifications/refresh] email status check failed for ${log.id}`, e);
    }
  }
  return { checked, updated };
}
...
/**
 * POST /api/admin/notifications/refresh
 *
