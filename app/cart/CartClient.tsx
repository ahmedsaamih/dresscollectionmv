...
import type { FixedLineItem, Product, StoreCollection, StorefrontCopy, StoreSetting } from '@/lib/types';
...
export function CartClient({ settings, collections, products }: CartClientProps) {
  useReveal();
  const { cart, refresh } = useCart();
  const navCopy = settings.storefrontCopy.homepageNavigation;
  const copy = settings.storefrontCopy.cartQuoteStatus;
  const router = useRouter();
  const fmt = formatMVR;
  const fixed = cart.fixed;
  const hasFixed = fixed.length > 0;
  const subtotal = fixed.reduce((a,i) => a + i.price * i.qty, 0);
  const hasPreOrder = fixed.some(i => products.find(p => p.id === i.sku)?.preOrder);
  ...
  const stockFor = (i: FixedLineItem) => {
    const product = products.find(p => p.id === i.sku);
  ...
  const totalStockFor = (i: FixedLineItem) => products.find(p => p.id === i.sku)?.colorSizeStock?.[i.color]?.[i.size] ?? 0;
                ...
                <div className="flex flex-col gap-4">
                  {fixed.map(i => (
                    <div key={i.id} data-motion-card className="flex gap-[16px] pb-4 border-b border-[rgba(0,0,0,.07)]">
                      <ProductImage img={i.img} alt={i.name} className="w-[86px] h-[108px] rounded-[12px] flex-none relative overflow-hidden" sizes="86px" />
            ...
            <div className="grid grid-cols-1 min-[380px]:grid-cols-2 gap-4 p-5">
              {accessories.map(p => (
                  <div key={p.id} className="flex flex-col">
                    <ProductImage img={p.img} alt={p.name} className="aspect-square rounded-[14px] relative flex-none overflow-hidden" sizes="200px" />
