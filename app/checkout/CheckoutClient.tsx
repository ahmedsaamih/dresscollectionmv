...
import type { DeliveryArea, Product, StoreCollection, StorefrontCopy, StoreSetting } from '@/lib/types';
...
export function CheckoutClient({ settings, collections, deliveryAreas, products }: CheckoutClientProps) {
  const { cart, refresh } = useCart();
  const navCopy = settings.storefrontCopy.homepageNavigation;
  const copy = settings.storefrontCopy.paymentCheckout;
  const shippingCopy = settings.storefrontCopy.shippingPickup;
  ...
  const preOrderSubtotal = cart.fixed.reduce((a,i) => {
    const p = products.find(pp => pp.id === i.sku);
  ...
  if (placed && conf) return (
    <div className="min-h-screen bg-page text-body font-archivo">
      ...
      <div className="max-w-[620px] mx-auto px-5 sm:px-8 py-[70px] text-center">
        ...
        <div className="text-left bg-[rgba(219,87,149,.04)] border border-[rgba(219,87,149,.16)] rounded-[14px] p-[18px_20px] mt-[18px]">
          ...
          {(settings.bankAccounts ?? []).length > 0 ? (
            ...
            <div className="text-[13px] text-sub">{settings.bank}</div>
          )}
        </div>
      ...
      <Footer tagline={settings.tagline} collections={collections} navCopy={navCopy} paymentCopy={copy} />
    </div>
  );
              ...
              <div className="flex flex-col gap-[14px]">
                <div>
                  ...
                  <select value={deliveryAreaId} onChange={e=>{ setDeliveryAreaId(e.target.value); setErrors(er=>({ ...er, deliveryAreaId:false })); }}
                    ...
                    {deliveryAreas.map(a => <option key={a.id} value={a.id}>{a.name} — {formatMVR(a.rate)}</option>)}
                  </select>
            ...
            <div className="flex flex-col gap-[11px] max-h-[230px] overflow-auto">
              {cart.fixed.map(i=>(
                <div key={i.id} className="flex gap-[11px] items-center">
                  <ProductImage img={i.img} alt={i.name} className="w-[46px] h-[46px] rounded-[9px] flex-none relative overflow-hidden" sizes="46px">
